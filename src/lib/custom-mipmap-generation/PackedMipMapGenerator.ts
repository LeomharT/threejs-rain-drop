import { Color, MathUtils, NearestFilter, ShaderMaterial, Texture, WebGLRenderer, WebGLRenderTarget } from 'three';
...
export class PackedMipMapGenerator {
  public _swapTarget: WebGLRenderTarget;
  public _copyQuad: FullScreenQuad;
  public _mipQuad: FullScreenQuad;
  public _mipMaterials;
  ...
  constructor(mipmapLogic = '') {
    ...
    this._mipQuad = new FullScreenQuad(undefined);
  ...
  update(
    ...
    if (forcePowerOfTwo) {
      width = MathUtils.floorPowerOfTwo(image.width);
      height = MathUtils.floorPowerOfTwo(image.height);
    } else {
    ...
    if (swapTarget.texture.type !== target.texture.type) {
      ...
      swapTarget.texture.image = { ...(swapTarget.texture.image as HTMLImageElement) };
    } else {
